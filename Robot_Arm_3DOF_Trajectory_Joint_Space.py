...
# Compute joint angles at a single timestep along a joint-space trajectory for a 3DOF arm

def joint_space_3dof(start_thetas, target_xy, lengths, T, dt):

    # Get target joint angles from IK for the final end-effector position

    target_thetas = ik_3dof(target_xy, lengths)[1]
    trajectory = []
    t = 0
...
def animate_3dof(traj, lengths, interval=50):
    ...
    fig.ani.save("Robot_Arm_3DOF_Trajectory_Joint_Space.gif", writer='pillow', fps=30)
